...
func TestVerifyVoteCircuit(t *testing.T) {
	...
	_, assigments, err := GenInputsForTest([]VoterTestData{
		{
			PrivKey: privKey,
			PubKey:  pubKey,
			Address: address,
	...
	now := time.Now()
	assert.SolvingSucceeded(CircuitPlaceholder(), &assigments[0],
...
func TestMultipleVerifyVoteCircuit(t *testing.T) {
	...
	_, assigments, err := GenInputsForTest(data, nil)
	c.Assert(err, qt.IsNil)
	assert := test.NewAssert(t)
	now := time.Now()
	for i, assigment := range assigments {
		c.Logf("proof %d of %d", i+1, len(assigments))
		err := test.IsSolved(CircuitPlaceholder(), &assigment, ecc.BLS12_377.ScalarField())
		assert.NoError(err)
	}
	fmt.Println("proving tooks", time.Since(now))
}

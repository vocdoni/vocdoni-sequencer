...
func (c *VerifyVoteCircuit) CheckCircomProof(api frontend.API) ([]frontend.Variable, error) {
	...
	circomInputsHash := c.CircomPublicInputsHash.Public[0]
	...
	circomPubPrivInputs := []frontend.Variable{
		...
		c.MinTotalCost, c.CostExp, c.CostFromWeight, c.UserWeight, c.ProcessId,
	...
	circomPubPrivInputs = append(circomPubPrivInputs, flatEncryptedBallot...)
	...
	circomHash.Write(circomPubPrivInputs...)
	elemSum := emulated.ValueOf[sw_bn254.ScalarField](circomHash.Sum())
	api.AssertIsEqual(len(circomInputsHash.Limbs), len(elemSum.Limbs))
	for i, v := range c.CircomPublicInputsHash.Public[0].Limbs {
		api.AssertIsEqual(v, elemSum.Limbs[i])
	}
	...
	return circomPubPrivInputs, verifier.AssertProof(c.CircomVerificationKey, c.CircomProof,
...
func (c *VerifyVoteCircuit) Define(api frontend.API) error {
	...
	circomInputs, err := c.CheckCircomProof(api)
	if err != nil {
		return err
	}
	...
	inputsHash, err := hashFn(api, append(circomInputs, c.Address, c.CensusRoot)...)
	if err != nil {
		return err
	}
	api.AssertIsEqual(c.InputsHash, inputsHash)
	// check the signature of the inputs hash
	msg := emulated.ValueOf[emparams.Secp256k1Fr](c.InputsHash)
	...
	if err := arbo.CheckInclusionProof(api, hashFn, censusAddress, c.UserWeight,
		c.CensusRoot, c.CensusSiblings[:]); err != nil {
		return err
	}

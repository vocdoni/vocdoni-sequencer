...
type AggregatorCircuit struct {
	...
	Proofs               [circuits.VotesPerBatch]stdplonk.Proof[sw_bls12377.ScalarField, sw_bls12377.G1Affine, sw_bls12377.G2Affine]
	BaseVerificationKey  stdplonk.BaseVerifyingKey[sw_bls12377.ScalarField, sw_bls12377.G1Affine, sw_bls12377.G2Affine] `gnark:"-"`
	VerificationKey      stdplonk.CircuitVerifyingKey[sw_bls12377.ScalarField, sw_bls12377.G1Affine]                    `gnark:"-"`
	DummyVerificationKey stdplonk.CircuitVerifyingKey[sw_bls12377.ScalarField, sw_bls12377.G1Affine]                    `gnark:"-"`
}
...
func (c AggregatorCircuit) checkProofs(api frontend.API, hashes circuits.VotersHashes) {
	...
	verifier, err := stdplonk.NewVerifier[sw_bls12377.ScalarField, sw_bls12377.G1Affine, sw_bls12377.G2Affine, sw_bls12377.GT](api)
	if err != nil {
	...
	// calculate the witness for each voter
	witnesses := []stdplonk.Witness[sw_bls12377.ScalarField]{}
	for i := 0; i < len(c.Proofs); i++ {
		...
		witnesses = append(witnesses, calculatedWitness)
	}
	// verify all the proofs
	if err := verifier.AssertDifferentProofs(c.BaseVerificationKey,
		[]stdplonk.CircuitVerifyingKey[sw_bls12377.ScalarField, sw_bls12377.G1Affine]{
			c.DummyVerificationKey, c.VerificationKey,
		}, validProofs, c.Proofs[:], witnesses,
	); err != nil {
		circuits.FrontendError(api, "failed to verify proofs", err)
	}
}
